...
# Admin chat ID for order DMs (numeric or @handle)
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Stage: handling 'other' crypto input
    ...
    if user_id in CART and CART[user_id].get('payment_method') and 'ig_handle' not in CART[user_id]:
        ...
        # Determine chat ID type
        chat_id = ADMIN_CHAT_ID
        try:
            # convert numeric ID if possible
...
async def handle_callback_query(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("strain_"):
        await send_strain_details(update, context, data.split("strain_",1)[1])
        return
    ...
    if data == "view_strains":
        ...
        await query.message.reply_text(
            f"🛒 *Your Cart*\n\n{cart_summary}", parse_mode=ParseMode.MARKDOWN,
            reply_markup=InlineKeyboardMarkup([
...
async def handle_callback_query_from_text(update, data):
    class DummyCQ:
        def __init__(self, update, data): self.message=update.message; self.data=data
