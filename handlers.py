import os import json import re import logging from telegram import InlineKeyboardButton, InlineKeyboardMarkup, Update from telegram.ext import ContextTypes from telegram.constants import ParseMode
...
with open("strains.json", "r") as f: STRAINS = json.load(f)
...
• Orders ship within 14 days of payment unless otherwise stated. • Worldwide shipping. • Strains sourced from breeders, seed hunts, or trusted nurseries. • 7-day satisfaction guarantee. • 1 free reship allowed (then customer covers shipping). • All clones are grown in Oasis root cubes. """

PRICING_TEXT = """ Pricing: • 1–2 clones: $80 each • 3+ clones: $60 each

Shipping: • USA: $40 (1–2 days) • International: $100 (3–5 days)

PayPal Fee: • +5% (applies to total including shipping) """

def calculate_price(items, country, payment_method): total_clones = sum(item['quantity'] for item in items) clone_price = 60 if total_clones >= 3 else 80 base_total = total_clones * clone_price shipping = 100 if country.lower() != "usa" else 40 fee = 0.05 * (base_total + shipping) if payment_method == "PayPal" else 0 return base_total + shipping + fee

async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE): user_id = update.effective_user.id CART[user_id] = {"items": []} buttons = [ [InlineKeyboardButton("📋 View Strains", callback_data="view_strains")], [InlineKeyboardButton("🛒 View Cart", callback_data="view_cart")], [InlineKeyboardButton("❓ FAQ", callback_data="faq")] ] await update.message.reply_text( "Welcome to Clone Direct! 🌱👋 Browse elite clones and build your custom order below.", reply_markup=InlineKeyboardMarkup(buttons) )

def slugify(name: str) -> str: # Replace spaces and special chars for callback_data return re.sub(r"[^a-zA-Z0-9_]+", "", name.replace(' ', ''))

def get_strain_buttons(): buttons = [] for i in range(0, len(STRAINS), 2): row = [] for j in range(2): if i + j < len(STRAINS): s = STRAINS[i + j] slug = slugify(s['name']) row.append(InlineKeyboardButton(s['name'], callback_data=f"strain_{slug}")) buttons.append(row) return buttons

async def send_strain_details(update: Update, context: ContextTypes.DEFAULT_TYPE, slug: str): # Convert slug back to name name = slug.replace('_', ' ') strain = next((s for s in STRAINS if s['name'] == name), None) if not strain: logger.error(f"Strain not found for slug: {slug}") return
...
async def handle_add_quantity(update: Update, context: ContextTypes.DEFAULT_TYPE): await update.callback_query.message.reply_text( "How many clones would you like to add?\n\n" + PRICING_TEXT )
...
async def handle_callback_query(update: Update, context: ContextTypes.DEFAULT_TYPE): query = update.callback_query await query.answer() user_id = update.effective_user.id data = query.data logger.info(f"Callback data received: {data}")
...
if data == 'view_strains':
    ...
    mapping = {'pay_btc':'Bitcoin (BTC)','pay_eth':'Ethereum (ETH)','pay_sol':'Solana (SOL)','pay_usdc':'USDC','pay_usdt':'USDT'}
...
async def handle_callback_query_from_text(update, data): class DummyCQ: def init(self, update, data): self.message = update.message; self.data = data async def answer(self): pass await handle_callback_query(Update(update.update_id, callback_query=DummyCQ(update, data)), None)
