    ...
    for i in range(0, len(STRAINS), 2):
        ...
        for j in range(2):
            ...
            if idx < len(STRAINS):
                ...
                # use numeric index so names with spaces/apostrophes never break
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # if waiting for IG handle after payment selection
    if user_id in CART and CART[user_id].get('payment_method') and 'ig_handle' not in CART[user_id]:
        ...
        telegram_user = update.effective_user.username or update.effective_user.id
        order_info = (
            f"📦 *New order*\n"
        ...
        try:
            ...
            # log but don’t crash
            logging.getLogger(__name__).error(f"Failed to DM order: {e}")
    ...
    # quantity entry after strain details
